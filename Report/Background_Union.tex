...
A lot of online services have done similar tasks before .Thus,creating and using an online database or automated metadata creation are to be recommended. \\\\\\\\\todo{Citation or a few examples needed9999999454546459.} The reason is there are many advantages, including integrating with the other cloud services or scaling with what users need such as how to categorize the categories.
...
Inspite of having a greater number of existing disambiguation algorithms, WSD still has an open problem with the three main parts of the WSD methods being considered by literature: Supervised, Unsupervised and semi-supervised. \todo{Merge this text into the rest in this section. I moved it from another place.}
...
\todo{Try to avoid speculations. If you speculate you should motivate it.} Support Vector Machines and memory-based learning have been shown to be the most successful approaches, to date, probably because they can cope with the high-dimensionality of the feature space. 
...
\todo{Good beginning. An example or explanation of the properties is needed. A citation on the last sentence is needed.}
...
The bootstrapping approach\todo{Which bootstraping? Missing text in between.} starts from a small amount of seed data for each word: either manually tagged training examples or a small number of surefire decision rules. 
...
The first sentence is simple future tense which imply something has never actually happened, the second sentence is simple present tense which can directly imply facts,
...
Without logic method, It is almost impossible to code and realized by computer.

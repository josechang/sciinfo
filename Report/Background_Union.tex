...
We are producing a program that automatically generate metadata, such as authors' name, date of publishing, name of articles, and, more importantly, an auto-generated abstract for full text articles.\todo{Please update based on what you are building}
...
\end{figure}
\todo{The figure is too small, it is hard to read the text, please make it cover two columns.}
...
The method to improve the problem above is "categorize the words based on different subjects,topic or genres" by using online database. Metadata is limited in digital libraries and web resources, try to enlarge them with meaningful, organized and desired categories \citep{Kules2006}.\todo{Check how to use citations in APA style.}
...
Also, an ambiguous word in one language is often translated into different words in a second language depending on the sense of the word. Word-aligned bilingual corpora have been used to infer cross-lingual sense distinctions, a kind of semi-supervised system\citep{Cheslow2014}.

\subsubsection*{Unsupervised}

Unsupervised WSD which rely on single writing can be approached by the use of Naive Bayes' model, which mainly focuses on unsupervised part of the context. In this model, a number of sentences are used which contains a particular word which has several meanings. The main goal is to divide those words into a specified number of sense groups \citep{4028513}.
...
Natural language generation (NLG) is one branch of natural language processing. The goal is generating the words human being using via machine automatically. To use this technique, six basic activities are done: (a) content determination, (b) discourse planning, (c) sentence aggregation, (d) lexicalization, (e) referring expression generation, and (f) linguistic realization \citep{Ramos-Soto2015}. Some of the activities are mentioned above.\todo{Where? Please explain the 6 terms. What are they?} The advantage of this technique is that it is flexible, since there is no standardization. But it also has the difficulty in the implication of this technique.\cite{Ramos-Soto2015}

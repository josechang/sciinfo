...
offers verity of funtionlity Whether y a fast growing web
 property, technology ISV, or large enterprise, MySQL can help developer
 to deliver high performance, scalable database applications and more predominently it has a free community. Cross platform and 
 thats why MYSQL is  good choice for  both development process
  which works on almost all the  platforms and for companies
   with heterogenous IT infrastructure.For  developmentwell MYSQL
...
To deal with large amounts of data , PostgreSQL is supported with  open-source third-party tools compatibility extensions for designing, managing, and applying the DBMS.
  Age of PostgreSQL is quite long when compared to other database. and It has well established  open acces  community through which large amount of  knowledge-transfer happens between experienced professional to amature developer. 
PostgreSQL not only  a RDBMS system but also have features of an OODBMS.
However, PostgreSQL is too simple may appear like less performance than MySQL.

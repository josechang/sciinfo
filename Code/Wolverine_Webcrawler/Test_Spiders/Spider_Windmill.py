...
def get_image_info(client, my_massage):
	...
	image_info = {'name': soup.find(id='caption_region').h3.string, 'link': urlparse.urljoin('http://www.nasa.gov', soup.find(attrs='Full_Size')['href'])}
	return image_info
...
def test_scrape_iotd_gallery():
	...
	for image_info in sorted(images_info.values(), key=lambda image_info: image_info['name']):

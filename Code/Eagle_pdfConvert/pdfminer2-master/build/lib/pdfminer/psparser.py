...
class PSStackParser(PSBaseParser):
    ...
    def start_type(self, pos, thisType):
        self.context.append((pos, self.curtype, self.curstack))
        (self.curtype, self.curstack) = (thisType, [])
        if 2 <= self.debug:
            print >> sys.stderr, 'start_type: pos=%r, thisType=%r' % (pos, thisType)
        return

    def end_type(self, thisType):
        if self.curtype != thisType:
            raise PSTypeError('Type mismatch: %r != %r' % (self.curtype, thisType))
        objs = [obj for (_, obj) in self.curstack]
        (pos, self.curtype, self.curstack) = self.context.pop()
        if 2 <= self.debug:
            print >> sys.stderr, 'end_type: pos=%r, thisType=%r, objs=%r' % (pos, thisType, objs)
        return pos, objs
